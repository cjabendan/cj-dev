...
export default function TechStackPage() {
  return (
    <div className="space-y-6 animate-fade-in mb-auto">
      <h1 className="text-xl sm:text-2xl font-bold">Tech Stack</h1>
          ...
          <div key={group.category} className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              {group.skills.map((skill) => (
                <div
                  key={skill.name}
                  className="flex items-center gap-3 px-3 py-2 border border-gray-200/60 dark:border-gray-800/80 rounded-sm bg-card hover:bg-muted/50 transition-colors"
                >
                  <Image
                    src={skill.icon}
                    alt={skill.name}
